...
def run(arguments):
    ...
    # Remove any empty strings or whitespace-only items
    ...
    x = get_bert_embeddings(sentences).numpy()
    print(x.shape)
    ...
    Sig = np.cov(x.T) + 1e-3 * np.eye(x.shape[1])
    ...
    for m in range(Ms.shape[0]):
      ...
      print("Coreset Indices:", idcs)
      print("Coreset Weights:", wts)

      # If you want to print the corresponding sentences:
      print("Coreset Sentences:")
      for idx, w1 in zip(idcs, wts):
          print(f"  [{w1:.4f}] {sentences[idx]}")

      with open("bert_coreset.txt", "w") as f:
        for idx, w1 in zip(idcs, wts):
            f.write(f"[{w1:.4f}] {sentences[idx]}\n")
    ...
    print("Total Time Taken: ", sum(times) / len(times))
